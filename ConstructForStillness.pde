...
// UPDATED: Increased from 0.02 to 0.15 for tighter visual tracking
...
void draw() {
  ...
  float bar_x = (width / 2.0) - (BAR_WIDTH / 2.0) + comp_offset; 
...
void serialEvent(Serial p) {
  ...
  if (line.startsWith("POS:")) {
    ...
    if (state == STATE_CENTERING) {
      ...
      // UPDATED: No more recalibrate state. It just immediately turns around!
      current_position = 0; 
...
void draw_debug(float bar_x, float comp_offset) {
  ...
  text("Screen Offset: " + nf(comp_offset, 0, 1) + " px", 20, y); y += 22;
  ...
  fill(BAR_COLOR); 
  float bar_screen_x = 30 + (pct * 460); 
  ellipse(bar_screen_x, y, 8, 8); 
  text("Bar Target", bar_screen_x - 25, y + 15);
  
  y += 50;
  fill(150, 200, 255);
  text("KEYS: +/-=Comp | [/]=Width | R=Home | Q=Quit | D=Debug", 20, y);
}
...
void loadSettings() {
  ...
  if (f.exists()) {
    ...
    println("Loaded settings: Width=" + BAR_WIDTH + ", Comp=" + compensation);
  } else {
